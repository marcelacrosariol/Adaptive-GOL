...
int main(int argc, char* argv[]){
    ...
    if (devCount > 1){
        ...
        int numThreads, *h_subGrid, *h_SubGridSize;
        int *d_subTemp;
        size_t subBytes;
        
        omp_set_num_threads(devCount);
        numThreads = omp_get_num_threads(); // Get the number of CPU threads
        
        for (iter = 0; iter < lim; iter ++){
            int numThreads, CPUthreadId, currentDevice, firstRow, lastRow;
            
            #pragma omp parallel private(CPUthreadId, h_subGrid, d_subGrid, d_subTemp, subBytes, firstRow, lastRow) shared(h_grid, gridBytes)
            {
                ...
                subBytes = sizeof(int)*(h_SubGridSize[currentDevice]*DIM);
                h_subGrid = (int*)malloc(subBytes); //allocate memory for the subGrid

                h_SubGridSize = (int*)malloc(sizeof(int)*devCount); // Allocate memory for the subGridSize, which stores the number of elements in each subGrid
                RowCount(devCount, h_SubGridSize); // Calculate the size of the subgrid in each GPU
                
                cudaMalloc(&d_grid, gridBytes); // Allocate memory for the grid in each GPU
                // Allocate device grids - if there is more than 1 thread, It'll allocate memory in each device
                cudaMalloc(&d_subGrid, subBytes);
                cudaMalloc(&d_newGrid, gridBytes);
                
                // copy the host grid to each GPU
                cudaMemcpy(d_grid, h_grid, gridBytes, cudaMemcpyHostToDevice);

                if(currentDevice == 0){ //do it only once
                    // for each iteraction of the game, calculate the ghost cells
                    ghostRows<<<cpyGridRowsGridSize, cpyBlockSize>>>(d_grid);
                    ghostCols<<<cpyGridColsGridSize, cpyBlockSize>>>(d_grid);
                }
                ...
                // call GOL function and the new values will go to the subTemp grid
                //GOL<<<gridSize, blockSize>>>(d_subGrid, d_subTemp, h_SubGridSize[currentDevice]);
                
                // concatenate the d_newGrid with the d_subTemp of each GPU
                //cudaMemcpy(d_newGrid + rowsBefore(currentDevice, h_SubGridSize), d_subTemp, subBytes, cudaMemcpyDeviceToDevice);
