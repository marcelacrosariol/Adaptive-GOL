...
int main(int argc, char* argv[]){
    ...
    if (devCount > 1){
        ...
        int device, firstRow, lastRow, rank, size, processFirstRow;
        ...
        for (iter = 0; iter < lim; iter ++){
			...
			if (rank == 0){
				...
				processFirstRow = rowsBefore(rank, DIM);
				
				for (i = 1; i< size; i++){
					MPI_Send(&h_grid[processFirstRow], DIM*DIM/size, MPI_INT, i, i, MPI_COMM_WORLD);  // Send a fragment of h_grid for each process
					
					MPI_Recv(&h_grid[processFirstRow], DIM*DIM/size, MPI_INT, i, 99, MPI_COMM_WORLD, &status);  // Receive the fragment that contains the resulting cells after the iteration 
				}
			}
			else{
				
				MPI_Recv(&h_grid, DIM*DIM/size, MPI_INT, 0, 99, MPI_COMM_WORLD, &status);
				...
				for (device = 0; device < devCount; device++){
                    ...
                    for (i = 1; i <= DIM; i++){
                        for (j = 1; j <= DIM; j++){
                            h_grid[i * (DIM+2) + j] = h_tempGrid[i * (DIM+2) + j];
                        }
                    }
				...
				MPI_Send(&h_grid, DIM*DIM/size, MPI_INT, 0, i, MPI_COMM_WORLD);
